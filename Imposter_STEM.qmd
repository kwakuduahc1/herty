...
@tbl-imp shows the shows the association between the demographic characteristics of the respondents and the feeling of impostor syndrome. The mean difference in the score of imposterism between respondents aged between 21 to 25 and 15 to 20 was `r inline_text(impmod, variable = age, level = 2)` and those 26 or more and those 15 to 20 was `r inline_text(impmod, variable = age, level = 3)`. However, none of the differences were statistically significant. On the average, students pursuing a mathematics or science program had a higher score, `r inline_text(impmod, variable = area, level = 2)`, compared to those pursuing a program in engineering or technology but this was not statistically significant. This means students pursuing mathematics or science a higher feeling of imposterism.
```{r}
#| label: tbl-imp
