...
```{r}
impmod <- dset %>% 
  select(imp_score, age:year) %>% 
  tbl_uvregression(method = glm, y = imp_score, hide_n = T)

```
@tbl-imp presents the results of the univariate linear regression models showing the association between the demographic characteristics of the respondents and the feeling of impostor syndrome. Compared to respondents aged 15 to 20 years, those between 21 to 25 had `r inline_text(impmod, variable = age, level = 2)` lower impostor score whereas those aged 26 years or above had `r inline_text(impmod, variable = age, level = 3)` higher impostor score. However, the differences were not statistically significant. Moreover, students in the areas of Mathematics and Science had higher scores indicating a stronger feeling of impostorism [`r inline_text(impmod, variable = area, level = "Mathematics and Science")` and `r inline_text(impmod, variable = area, level = "Engineering and Technology")` respectively] compared to those in Engineering and Technology and the mean differences were statistically significant. Additionally, respondents in third year had significantly higher impostor score compared with those in first year `r inline_text(impmod, variable = year, level = "Third Year")`.
```{r}
#| label: tbl-imp
...
#### Multiple linear regression showing the association between composite score of imposterism and predictor variables.

```{r}
mlinearreg <- glm(formula = imp_score ~ age + area + year + age*area +area*year, 
                  data = dset, family = gaussian("identity")) %>% 
  tbl_regression()
```


#### Association between demographic variables and the composite score of imposterism
In the multiple linear regression model, students aged 21-25 had significantly lower impostor scores [`r inline_text(mlinearreg, variable = age, level = 2)`] after adjusting for other covariates, indicating lesser feeling of the impostor syndrome. The analysis also found statistically significant interaction between age group and study area. Specifically, students aged 21-25 years who are in the area of Mathematics and Science had `r inline_text(mlinearreg, variable = "age:area", level = 1)` higher impostor scores, indicating greater feeling of impostor syndrome. FOr other factors (regressors) including study areas and level alone, no signifcant mean differences were observed across their levels.

```{r}
glm(formula = imp_score ~ age * year, data = dset, family = gaussian(link = "identity")) %>% 
  tbl_regression(
    intercept = T
  )
```
