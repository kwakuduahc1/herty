...
impd <- dset %>% 
  mutate(
    across(eval: study_unable, as.numeric),
    imp_score = (eval + do_prog + impress + afraid + luck + found + study_unable)/35*100
  ) %>% 
  select(age:year, imp_score)

impmod <- impd %>% 
  tbl_uvregression(method = glm, y = imp_score)
  
```

@tbl-imp is a univariate linear regression model that shows the association between the demographic characteristics of the respondents and the feeling of impostor syndrome. The mean difference in scores of imposterism between respondents aged between 21 to 25 and 15 to 20 was `r inline_text(impmod, variable = age, level = 2)` and those 26 or more and those 15 to 20 was `r inline_text(impmod, variable = age, level = 3)`. None of the differences were statistically siginificant. Females students in mathematics and science had a higher feeling of impostor syndrome compared to those in engineering and the differences in mean were statistically significant. Those in science had a mean increase of `r inline_text(impmod, variable = area, level = "Science")` in the impostor score but it was not statistically significant. 
```{r}
#| label: tbl-imp
